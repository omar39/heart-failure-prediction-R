...
```{r}
ggplot(data, aes(x=factor(sex), y=age, color=factor(sex))) + geom_boxplot(outlier.color = 'red', outlier.shape = 8) + geom_jitter(shape=16, position=position_jitter(0.2)) + labs(x='sex', title='Distribution of gender and their ages', color='sex') + scale_color_discrete(labels=c('Female', 'Male'))
```
...
taining.samples <- data$DEATH_EVENT %>%
  createDataPartition(p=0.5, list=FALSE) # splitting the data 50-50, to avoid over fitting
